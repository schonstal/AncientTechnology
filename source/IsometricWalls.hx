...
class IsometricWalls
{
  ...
  private function cutLakes():Void {
    for(j in 0...10) { 
      position.x = new FlxRandom().int(0, dungeonTiles.width - (BRUSH_SIZE + 1));
      position.y = new FlxRandom().int(0, dungeonTiles.height - (BRUSH_SIZE + 1));
      ...
      for(i in 0...10) {
        ...
        var localSize = BRUSH_SIZE + (new FlxRandom().bool(30) ? 1 : 0);
  ...
  private function changeDirection():Void {
    if (new FlxRandom().bool(40) || outOfBounds()) {
